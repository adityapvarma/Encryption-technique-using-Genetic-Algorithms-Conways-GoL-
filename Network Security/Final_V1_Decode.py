...
en_text=input("Enter Encrypted Text :")
en_text=en_text.upper()

ct_pt=en_text.count("|")+1
buff_text=''

while len(buff_text)<ct_pt:
    buff_text+=key

buff_text=buff_text[:ct_pt]
#print(buff_text)
buff_text=buff_text.upper()
...
for i in range(len(buff_text)):
    r=ord(buff_text[i])-32

    for j in range(len(obj_list[r])):
        if obj_list[r][j].value==(ord(en_l[i][0])-32) and obj_list[r][j].sub==int(en_l[i][1:]):
            dec_text+=chr(j+32)
            #print(j)
            break
...
for i in range(len(en_l)):
    r=ord(buff_text[i])-32
    for j in range(len(obj_list[r])):
        if obj_list[r][j].value==en_l[i][0] and obj_list[r][j].sub==int(en_l[i][1:]):
            c=j
            dec_text+=chr(c+32)
            break
    #dec_text+=chr(c+32)
