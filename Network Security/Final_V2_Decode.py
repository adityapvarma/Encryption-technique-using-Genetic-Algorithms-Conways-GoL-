...
enc_ct=enc_text.count("|")+1

enc_l=enc_text.split("|")
    ...
    for i in range(len(key)):
        ...
        if over_ct<enc_ct:
            ...
            for j in range(len(obj_list[r])):
                ...
                if obj_list[r][j].value==(ord(enc_l[(mul*len(key))+i][0])-32) and obj_list[r][j].sub==int(enc_l[(mul*len(key))+i][1:]):
                    pt+=chr(j+32)
                    over_ct+=1
                    #print(over_ct)
                    break
